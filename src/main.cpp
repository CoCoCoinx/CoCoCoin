...
#if defined(NDEBUG)
# error "CoCoCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x00000a27555956b9e93c205997aa0795b3ce95e73b441f9a27176edb54a2ba82");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CoCoCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CoCoCoin Signed Message:\n";
...
{
    // CoCoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
	if(nHeight<1) return  330000 * COIN;
	
	double dDiff = ConvertBitsToDouble(nBits);
	
	int64 nSubsidy  = (2222222.0 / (pow((dDiff+2600.0)/9.0,2.0)));
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CoCoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CoCoCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CoCoCoinCoCoCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1403704458;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1051558;
        ...
        assert(block.hashMerkleRoot == uint256("0x6be1c606aeee31b43b7720c7c8a2f89e60f6dbe80537c3ee78724c5e4efa67c0"));
...
unsigned char pchMessageStart[4] = { 0x1c, 0x11, 0x17, 0x1c }; 
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoCoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoCoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoCoCoinMiner(CWallet *pwallet)
{
    printf("CoCoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cococoin-miner");
    ...
    try { loop {
        ...
        printf("Running CoCoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoCoCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoCoCoinMiner, pwallet));
}
